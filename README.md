...
- building a **pipeline for filtering out false positive oligonucleotide mentions**. Its need arose as many extracted sequences follow structural patterns of an oligonucleotide but were actually gene expressions, mutations, variations, etc. Thus to further filter out false positives generated by Regex based oligonucleotide mentions and oligonucleotide names extraction pipeline, we need such tagging/curation system. 
  This objective is further sub-divided into:-
   ...
   - developing **BOW based pipeline to auto-curate extracted sequences** to be True positive oligonucleotide mentions or False positive oligonucleotide mentions. This pipeline will also reduce efforts to manual curate sequences as TP or FP. To keep improving this pipeline, a **pipeline for automatic advancement/improvisation of BOWs** needs to be developed. This pipeline will use manual curations done by curator on the CSV file to extend number of words in the existing BOWs.
...
- preparing a **high-quality golden standard dataset of oligonucleotide mentions** using:-
   ...
   This dataset will be stored in databases like WormBase and then used by Biomedical researchers to take reference for use of specific oligonucleotide in experiments and to validate the performance of future developments of oligonucleotide extraction systems.
